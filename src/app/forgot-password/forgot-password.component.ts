...
export class ForgotPasswordComponent {
  ...
  verifyPhoneNumber(phoneNumber: number){
    ...
    this.http.get(`http://localhost:5001/api/check-phone?phoneNumber=${phoneNumber}`).subscribe((res:any) => {
      if(res.exists){
  ...
  setNewPassword(phone: number, password: string, confirmPassword: string){
    ...
    this.http.post('http://localhost:5001/api/reset-password', {phoneNumber: phone, newPassword: password}).subscribe((res:any) => {
